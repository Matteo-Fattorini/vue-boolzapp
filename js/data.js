

const contacts = [
  //array of contacts, as objs.
  {
    avatar: "css/img/avatar_io.jpg",
    name: "Noemi",
    lastSeen: "22:32",
    messages: [
      {
        text: "Hey what's going on?",
        timeStamp: "22.15",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Everything's good!",
        timeStamp: "22.15",
        isSent: true,
        isArrowClicked: false,
      },
      {
        text: "Wanna hang up later?",
        timeStamp: "22.18",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Sure!",
        timeStamp: "22.19",
        isSent: true,
        isArrowClicked: false,
      },
    ],
    id: 100,
  },
  {
    avatar: "css/img/avatar_1.jpg",
    name: "Jacopo",
    lastSeen: "21:00",
    messages: [
      {
        text: "Hey man do you like this app?",
        timeStamp: "21.15",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "I love it!",
        timeStamp: "21.15",
        isSent: true,
        isArrowClicked: false,
      },
      {
        text: "Do you think they are gonna patch it?",
        timeStamp: "21.18",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Not sure about that!!",
        timeStamp: "21.19",
        isSent: true,
        isArrowClicked: false,
      },
    ],
    id: 101,
  },
  {
    avatar: "css/img/avatar_3.jpg",
    name: "Leo",
    lastSeen: "20:50",
    messages: [
      {
        text: "It's so hot today!",
        timeStamp: "19.15",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Indeed!",
        timeStamp: "19.15",
        isSent: true,
        isArrowClicked: false,
      },
    ],
    id: 102,
  },
  {
    avatar: "css/img/avatar_4.jpg",
    name: "Marco",
    lastSeen: "19:00",
    messages: [
      {
        text: "Did you hear the news?",
        timeStamp: "18.15",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Yeah, unbelievable!",
        timeStamp: "18.15",
        isSent: true,
        isArrowClicked: false,
      },
    ],
    id: 103,
  },
  {
    avatar: "css/img/avatar_5.jpg",
    name: "Elia",
    lastSeen: "18:00",
    messages: [
      {
        text: "Hey, you heard what happened to Marco?",
        timeStamp: "17.15",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "No, what happened?",
        timeStamp: "17.15",
        isSent: true,
        isArrowClicked: false,
      },
      {
        text: "He is no longer engaged... ",
        timeStamp: "17.18",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Omg!",
        timeStamp: "17.28",
        isSent: true,
        isArrowClicked: false,
      },
    ],
    id: 104,
  },
  {
    avatar: "css/img/avatar_6.jpg",
    name: "Giulia",
    lastSeen: "16:00",
    messages: [
      {
        text: "Ow, i love kittens so much!",
        timeStamp: "15.15",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "I actually prefer dogs",
        timeStamp: "15.15",
        isSent: true,
        isArrowClicked: false,
      },
      {
        text: "They can't purr!",
        timeStamp: "15.18",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "But they can fetch!",
        timeStamp: "15.41",
        isSent: true,
        isArrowClicked: false,
      },
    ],
    id: 105,
  },
  {
    avatar: "css/img/avatar_7.jpg",
    name: "Alessio",
    lastSeen: "14:00",
    messages: [
      {
        text: "Wanna hear a joke?",
        timeStamp: "13.15",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Sure..!",
        timeStamp: "13.15",
        isSent: true,
        isArrowClicked: false,
      },
      {
        text:
          "What is the best thing about Switzerland?  I don't know, but the flag is a big plus!",
        timeStamp: "13.18",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Ahahah!",
        timeStamp: "13.36",
        isSent: true,
        isArrowClicked: false,
      },
    ],
    id: 106,
  },
  {
    avatar: "css/img/avatar_8.jpg",
    name: "Luca",
    lastSeen: "12:00",
    messages: [
      {
        text: "Such a rainy day today",
        timeStamp: "11.15",
        isSent: false,
        isArrowClicked: false,
      },
      {
        text: "Yeah, we had better days",
        timeStamp: "11.15",
        isSent: true,
        isArrowClicked: false,
      },
    ],
    id: 107,
  },
];

const possibleAnswers = [
  //possible answer that every contact can give
  "Not sure about that",
  "Yeah, whatever",
  "I guess so...",
  "Probably not",
  "Yeah, indeed!",
  "No, i don't think so",
  "Are you sure about that?",
  "I never tought about that!",
  "I would love to!",
  "I always wanted you to ask me that...",
  "Me too!",
];
